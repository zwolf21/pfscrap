...
def insert_db_table_kofia_price_progress(table_name=RW_FUNDINDEX_TABLE_NAME, info_table_name=FUNDINFO_TABLE_NAME, mapping=RW_FUNDINDEX, info_mapping=RW_FUNDINFO, **db_connection_info):
    ...
    펀드명 = info_mapping['펀드명']
    상환여부 = info_mapping['상환여부']
    기준일자 = mapping['기준일자']
    db = DBOrm(**db_connection_info)
    # DB의 기존 펀드리스트 테이블을 얻는다
    df_fund_list_table = db.get_df(
        info_table_name,
        columns=[표준코드, 설정일, 회사코드, 펀드명],
        where=f'{스크랩여부}="Y" AND {상환여부}="N"'  # 일단은 스크랩 여부 관계 없이 다 받아봄
    )
    # DB의 기존 지수테이블을 얻는다
    ...
    ##### 초기 데이터 설정시 2019-05-28 까지 데이터로 통일을 위한 필터링
    received = df_progress_table[표준코드].drop_duplicates().values
    not_received_mask = ~df_fund_list_table[표준코드].isin(received)
    df_fund_list_table = df_fund_list_table[not_received_mask]


    total_count = df_fund_list_table.shape[0]
    print('insert_db_table_kofia_price_progress test')
    print('지수정보를 업데이트할 건수', total_count)
    # print(df_progress_table)
    # 기존 펀드리스트 테이블을 순회하며 얻은 펀드 코드 값으로 각 코드마다 지정보를 구해 저장한다.
    # print(df_fund_list_table.head())


    for i, (index, row) in enumerate(df_fund_list_table.iterrows()):
        ...
        _print_db_progress(
            total_count, i, 1,
            prefix=f'{df.shape[0]} 개의 지수정보 저장중...({i}/{total_count}){fund_std_code}, {fund_nm} (조회시작일자:{지수조회시작일자})'
        )
        # DB 저장
